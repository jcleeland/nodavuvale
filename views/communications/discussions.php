...
$discussions = $db->fetchAll("SELECT discussions.*, users.first_name, users.last_name FROM discussions INNER JOIN users ON discussions.user_id=users.id ORDER BY is_sticky DESC, created_at DESC");
?>
                    ...
                    <div class="discussion-reactions" data-discussion-id="<?= $discussion['id'] ?>">
                        ...
                        <div class="reaction-summary-container">
                            ...
                            <div class="text-sm text-gray-500">
                                Posted on <?= date('F j, Y, g:i a', strtotime($discussion['created_at'])) ?> by <?= $discussion['first_name'] ?> <?= $discussion['last_name'] ?>
                            </div>
                        </div>
                    </div>
                    <?php if ($is_admin || $_SESSION['user_id'] == $discussion['user_id']): ?>
                        <form method="POST" class="mt-4">
                            <input type="hidden" name="discussion_id" value="<?= $discussion['id'] ?>">
                            <button type="submit" name="delete_discussion" class="text-red-500 hover:text-red-700">Delete Discussion</button>
                        </form>
                    <?php endif; ?>
                                ...
                                <div class="bg-gray-100 p-4 rounded-lg mt-2">
                                    ...
                                    <div class="comment-reactions" data-comment-id="<?= $comment['id'] ?>">
                                        ...
                                        <div class="reaction-summary-container">
                                            ...
                                            <div class="text-sm text-gray-500">
                                                - <?= htmlspecialchars($comment['first_name']) ?> <?= $comment['last_name'] ?> on <?= date('F j, Y, g:i a', strtotime($comment['created_at'])) ?>
                                            </div>
                                        </div>
                                    </div>
                                    <?php if ($is_admin || $_SESSION['user_id'] == $comment['user_id']): ?>
                                        <form method="POST" action="delete_comment.php" class="mt-2">
